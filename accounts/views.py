...
def register_view(request):
    ...
    if request.method == 'POST':  # user has submitted the form
        form = UserRegistrationForm(request.POST)   # fixed spelling
        if form.is_valid():
            user = form.save()
            login(request, user)
    ...
    # Always return render for both GET and invalid POST
...
def login_view(request):
    ...
    if request.method == 'POST': # user wants to register
        ...
        if form.is_valid():
            ...
            # if the user found not in db
            if user is not None:
                login(request, user)
                messages.success(request, f'Welcome back {user.username}!')
                return redirect('media_assets:dashboard')
    else:
        form = UserLoginForm(request) # default http method is GET
